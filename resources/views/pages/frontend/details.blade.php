  ...
  <section class="bg-gray-100 py-8 px-4">
    <div class="container mx-auto">
      <ul class="breadcrumb">
        ...
        <li>
          <a
            href="{{ url()->current() }}">{{ Str::length($product->name) > 15 ? Str::substr($product->name, 0, 15) . '...' : $product->name }}</a>
        </li>
        <li>
            ...
            @foreach ($product->galleries as $item)
              <div class="px-2">
                <div class="item {{ $loop->first ? 'selected' : '' }}" data-img="{{ Storage::url($item->url) }}">
                  <img src="{{ Storage::url($item->url) }}" alt="Foto {{ $loop->iteration . ' ' . $product->name }}"
          ...
          <div class="preview">
            <div class="item rounded-lg h-full overflow-hidden">
              <img
                ...
                alt="Foto {{ $product->name }}" class="object-cover w-full h-full rounded-lg" />
            </div>
          </div>
        </div>
      </div>
      <div class="flex-1 px-4 md:p-6">
        <h2 class="text-3xl font-semibold mb-2 text-slate-900">{{ $product->name }}</h2>
        <p class="text-3xl font-bold mb-4">Rp{{ number_format($product->price) }}</p>
        ...
        <div class="text-md leading-7 mb-6">
          {!! $product->description !!}
        </div>
      </div>
    </div>
